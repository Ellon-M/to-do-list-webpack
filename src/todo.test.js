import {
  mockStorage, saveToStorage, addToList, removeFromStorage, removeFromList, updateTask, updateCompleted, clearAllCompleted
...
describe('edit, update completed, clear all completed tasks', () => {
  test('edit task description in localstorage', () => {
    ...
    // Act
    updateTask([{index: 0, desc: 'task 2'}, {index: 1, desc: 'task 2'}], 'task 1', 0, {get, set});
  ...
  test('update completed status', () => {
    ...
    // Act
    updateCompleted([{index: 0, desc: 'task 1', completed: false}, {index: 1, desc: 'task 2', completed: false}], 0, { set });

    // Assert
    expect(setSpy).toHaveBeenCalled();
    expect(mockStorage.tasks[0]).toEqual({ index: 0, desc: 'task 1', completed: true});
  })
})